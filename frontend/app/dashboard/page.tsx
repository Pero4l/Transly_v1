...
import { PackagePlus, Box, Clock, ShieldAlert, Navigation } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
          {shipments.length === 0 ? (
            ...
            shipments.map((shipment: any) => (
              <Card key={shipment.id} className="border-0 shadow-sm rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 hover:shadow-md transition-all">
                ...
                <div className="flex flex-col items-end gap-3 justify-center pl-6">
                  ...
                  {user?.role === 'driver' && shipment.status !== 'delivered' && (
                     <div className="flex gap-2">
                       {shipment.status === 'assigned' && <Button size="sm" onClick={() => updateStatus(shipment.id, 'picked_up')}>Pick Up</Button>}
                       {shipment.status === 'picked_up' && <Button size="sm" onClick={() => updateStatus(shipment.id, 'in_transit')}>In Transit</Button>}
                       {shipment.status === 'in_transit' && <Button size="sm" variant="outline" className="border-emerald-500 text-emerald-600" onClick={() => updateStatus(shipment.id, 'delivered')}>Deliver</Button>}
                     </div>
                  )}
                </div>
              </Card>
