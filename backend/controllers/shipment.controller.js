...
const calculatePrice = async (distance) => {
  try {
    // Try to get specific shipment settings first
    let priceSetting = await Setting.findOne({ where: { key: 'SHIPMENT_PRICE_PER_KM' } });
    let baseSetting = await Setting.findOne({ where: { key: 'SHIPMENT_BASE_FARE' } });

    // Fallback to old generic settings if specific ones don't exist
    if (!priceSetting) priceSetting = await Setting.findOne({ where: { key: 'PRICE_PER_MILE' } });
    if (!baseSetting) baseSetting = await Setting.findOne({ where: { key: 'BASE_FARE' } });

    const rate = priceSetting ? parseFloat(priceSetting.value) : 500;
    const base = baseSetting ? parseFloat(baseSetting.value) : 1500;
    
    return (base + (distance * rate)).toFixed(2);
  } catch (err) {
    console.error('Price calculation error:', err);
    return (100 + (distance * 200)).toFixed(2);
  }
};
...
exports.getPricingSettings = async (req, res) => {
  try {
    const baseSetting = await Setting.findOne({ where: { key: 'SHIPMENT_BASE_FARE' } }) || await Setting.findOne({ where: { key: 'BASE_FARE' } });
    const priceSetting = await Setting.findOne({ where: { key: 'SHIPMENT_PRICE_PER_KM' } }) || await Setting.findOne({ where: { key: 'PRICE_PER_MILE' } });
    
    res.status(200).json({ 
      success: true, 
      settings: {
        BASE_FARE: baseSetting ? baseSetting.value : 1500,
        PRICE_PER_MILE: priceSetting ? priceSetting.value : 500
      } 
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
